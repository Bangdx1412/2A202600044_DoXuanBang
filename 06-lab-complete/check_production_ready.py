"""
Production readiness checker for Day 12 Lab.
...
def run_checks() -> bool:
    ...
    secrets_found = []
    for rel_path in ["app/main.py", "app/config.py", "app/auth.py", "app/rate_limiter.py", "app/cost_guard.py"]:
    ...
    results.append(
        check("No hardcoded secrets in code", len(secrets_found) == 0, str(secrets_found) if secrets_found else "")
    )
    ...
    if os.path.exists(main_py):
        ...
        results.append(check("Authentication implemented", "verify_api_key" in content or "api_key" in content.lower()))
        results.append(check("Rate limiting implemented", "check_rate_limit" in content or "429" in content))
        ...
        results.append(check("Redis-backed state", "get_redis_client" in content and "history:" in content))
    else:
    ...
    if os.path.exists(dockerfile):
        ...
        results.append(check("Non-root user", "useradd" in content and "USER agent" in content))
    ...
    if pct == 100:
        ...
        print("  Good progress - several items still need work.")
    else:
